...
void ZDrive_PVT_Plan(uint8_t id, float angle_start, float angle_end, 
                    float speed_max, float time_total) {
    ZDrive[id-1].PVT_Mode.angle0 = angle_start;
    ZDrive[id-1].PVT_Mode.targetangle = angle_end;
    ZDrive[id-1].PVT_Mode.targetSpeed = speed_max;
    ZDrive[id-1].PVT_Mode.targetTime = time_total;
    ZDrive[id-1].PVT_Mode.controlTime = 0;
    ZDrive[id-1].PVT_Mode.firstFlag = true;
    ZDrive[id-1].PVT_Mode.PVTModeFlag = true;
}
...
void ZDrive_Init() {
    for (int i = 0; i < USE_ZDRIVE_NUM; i++) {
        ZDrive[i].Param.reductionRatio = 6.f;
        ZDrive[i].Param.gearRatio = 2.f;
...
void ZDrive_PVT_Update(uint8_t id, float delta_time) {
    if (!ZDrive[id-1].PVT_Mode.PVTModeFlag) return;
    
    ZDrive[id-1].PVT_Mode.controlTime += delta_time;
    float t = ZDrive[id-1].PVT_Mode.controlTime / ZDrive[id-1].PVT_Mode.targetTime;
    
    if (t >= 1.0f) {
        // PVT完成
        ZDrive[id-1].PVT_Mode.PVTModeFlag = false;
        t = 1.0f;
    }
    
    // 简单的线性插值示例
    float current_angle = ZDrive[id-1].PVT_Mode.angle0 + 
                         (ZDrive[id-1].PVT_Mode.targetangle - ZDrive[id-1].PVT_Mode.angle0) * t;
    float current_speed = ZDrive[id-1].PVT_Mode.targetSpeed; // 简化
    
    // 发送PVT指令
    ZDrive_Set(Zdrive_PVT_Mode, id, current_speed, current_angle);
}
                ...
                switch (ZDrive[i].Mode) {
                    ...
                    default: 
                        break;
								...
								if (ZDrive[i].PVT_Mode.PVTModeFlag) {
                    ZDrive_PVT_Update(i+1, 10.0f); // 10s周期
								}
            }
          else {
            if (ZDrive[i].ModeRead != Zdrive_Position) {
